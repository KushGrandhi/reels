...
openai.api_key = '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'  # Replace with your actual OpenAI API key
...
def segment_video(response):
    ...
    for i, segment in enumerate(response):
        ...
        # Extract the subclip based on start_time and end_time
...
#             #print('problem')
#             break 
#         # img = cv2.imread('/Users/kush/Documents/Listed/temp 2/tree-with-blurred-background.jpg')
...
#         #mx = (0,0,target_width,target_height)      # biggest bounding box so far
#         mx = (0,0,0,0)
#         mx_area = 0
#         step = 10  #alternate fps to update
#         if n==1:
#             x,y,w,h = (0,0,target_width,target_height)
#             old_mx = (0,0,0,0)
#             #print(f'Added {n}')
#             images.append(img)
#         else:
...
#                 # Output to files
#                 my1,my2,mx1,mx2 = 0,0+target_height , x + round(w/2) - round(target_width/2),x + round(w/2) + round(target_width/2)
#                 if mx1>0:
#                     roi=img[my1:my2,mx1:mx2]
#                 else:
#                     roi = img[my1:my2,0:target_width]
#                 #roi2 = img[y:y+h,x:x+w]

#                 #print(0,0+target_height,abs(x + round(w/2) - round(target_width/2)),x + round(w/2) + round(target_width/2))
                
#                 #cv2.imshow('img',roi)
#                 #cv2.imshow('img2',roi2)
#                 #cv2.imwrite(f'frames/Image_crop{n}.jpg', roi)

#                 t2 = x + round(w/2)                 #final frame
#                 t1 = old_mx[0] + round(old_mx[2]/2) #initial frame 
#                 diff = (t2-t1)/step
#                 #print(f'\tInitial x: {t1},Final x: {t2}, adjust per frame: {diff}')
#                 if n!=1:
#                     for i in range(1,step):
#                         #print('UPFrame: ',n - step + i)
#                         fra = n - step + i
#                         y1,y2,x1,x2 = 0,0+target_height ,t1 - round(target_width/2) + round(i*diff),t1 + round(target_width/2)+round(i*diff)
#                         if x1>0:
#                             troi = images[i-1][y1:y2,x1:x2]
#                         else:
#                             troi = images[i-1][y1:y2,0:target_width]
#                         #print(f'\tXact: {t1 + round(i*diff)} ,X1: {(t1 -round(target_width/2) + round(i*diff))}, X2: {round(t1 + round(target_width/2)+round(i*diff))}')
#                         #print(troi)
#                         try:
#                             cv2.imwrite(f'frames/Image_crop{fra}.jpg', troi)
#                         except:
#                             print(f'frame{fra}: failed for values y1: {y1}, y2: {y2},x1: {x1}, x2: {x2}')
#                         targets.append(troi)
#                 #print(abs(x + round(w/2) - round(target_width/2)),x + round(w/2) + round(target_width/2))
#                 #print('Current: ',x,y,w,h)
#                 #print(roi)
#                 targets.append(roi)
#                 try: 
#                     cv2.imwrite(f'frames/Image_crop{n}.jpg', roi)
...
#     print(len(targets))
#     frames_to_video('/Users/kush/Documents/Projects/Code/AI-Shorts-Creator/frames',out, fps)
#     #frames_to_video('frames','output003.mp4','output_cropped003.mp4', fps)
#     cv2.waitKey(3)
#     os.system("rm -rf frames")

# for n,i in enumerate(targets):
#     cv2.imwrite(f'frames/Image_crop{n}.jpg', i)
#     print(f'Written: {n}')
    # try:
    #     cv2.imwrite(f'frames/Image_crop{n}.jpg', i)
    #     print(f'Written: {n}')
...
def get_human(img,show=False):
    
    results = model(img,save_conf=True,show =show,verbose=False)
    for result in results:
        boxes = result.boxes  # Boxes object for bbox outputs
        confs = boxes.data[:, 4:6] # Confidence and class ID of the detected objects
    
    #print(boxes.boxes)
    horizontal_centers = boxes.boxes[:, 0] + boxes.boxes[:, 2] / 2
    #print(confs)
    plots = boxes.boxes.tolist()
    #print(plots)
    res = list(zip(horizontal_centers,confs,plots))
    #print(res)
    formatted_list = [[int(t[0]), float(t[1][0]), int(t[1][1]),(int(t[2][0]),int(t[2][1]),int(t[2][2]),int(t[2][3]))] for t in res if int(t[1][1]) == 0]
    sorted_data = sorted(formatted_list, key=lambda x: x[1],reverse=True)
...
def reeler3(path,out):
    ...
    while vid.isOpened():
        ...
        # Calculate the target width and height for cropping (vertical format)
        target_height = int(frame_height * 1)
        target_width = int(target_height * 9/16)

        step = 100  #alternate fps to update
        if n==1:
            x,y,w,h = (round(frame_width/2)- round(target_width/2),0,target_width,target_height)
            old_x1 = x
            #print(f'Added {n}')
            images.append(img)
            old_im = img
            ...
            if n%step==0 or flag==True:
 
                result = get_human(img)
                if result != None:
                    if len(result)==1:
                        x,b,c,d = result[0][3]
                        last_hum = n
                    else:
                        x = round(frame_width/2)- round(target_width/2)
                else:
                    # diner = n-last_hum
                    # if diner>10:
                    x = round(frame_width/2)- round(target_width/2)
                ...
                my1,my2,mx1,mx2 = 0,0+target_height , x  ,x  + round(target_width)
                roi=img[my1:my2,mx1:mx2]

                if len(images) >0:
                    diff = (mx1-old_x1)/len(images)
                    for num,isc in enumerate(images):
                        frame = n - len(images) + num

                        #print(f'            {frame}')
                        troi=isc[my1:my2,old_x1+round(diff*num):old_x1+round(diff*num)+ target_width]
                        #cv2.imshow('img',troi)
                        cv2.imwrite(f'frames/Image_crop{frame}.jpg', troi)
                        #cv2.waitKey(1)
                else:
                    pass
                #print(f'                {n}')
                ...
                score = checksim(img, old_im)
                dim= n-last_n
                if score < 80 and dim>10:
                    #print('Similarity: ',score,n)
                    #cv2.imshow('current', img)
                    #cv2.imshow('previous', old_im)
                    #cv2.waitKey(1)
                    flag=True
                    last_n=n
                else:
...
#         mx = (0,0,0,0)
#         mx_area = 0
#         step = 30  #alternate fps to update
#         if n==1:
#             x,y,w,h = (0,0,target_width,target_height)
#             old_mx = (x,y,w,h)
#             old_x1 = x
...
#                 if w*h>5000:
#                     x,y,w,h = mx
#                     #x,y,w,h = (round(frame_width/2),0,target_width,target_height)
#                     #old_mx = (x,y,w,h)
#                 else:
#                     x,y,w,h = old_mx
#                     #x,y,w,h = (round(frame_width/2),0,target_width,target_height)
...
#                 cv2.imshow('fu',croi)
#                 # results = model(croi,show=True)
#                 # try:
#                 #     result = get_human(croi)  
#                 # except:
#                 #     result = [[0],[0]]
...
#                     print("tester",result) 
#                     #result =  result[0][0]
#                     print("Lengthener",len(result))
#                     if len(result)==1:
#                         dx,dy,dw,dh = get_face_coordinates(img)
#                         result = round(dx+(dw/2))
#                     else:
#                         hum_img = img[0:0+target_height , result-round(target_width/2) :result + round(target_width/2)]
#                         dx,dy,dw,dh = get_face_coordinates(hum_img)
#                         result = round(dx+(dw/2))
#                 except:
#                     try:
#                         result = get_human(img) 
#                         result =  result[0][0]
#                         #print("sadder",len(result))
#                     except:
#                         print("lol")
#                         result = None
#                     #print(len(result),) 
...
#                 # Output to files
#                 if result!=None:
#                     my1,my2,mx1,mx2 = 0,0+target_height , result-round(target_width/2) ,result + round(target_width/2)
#                 else:
#                     my1,my2,mx1,mx2 = 0,0+target_height , x + round(w/2) - round(target_width/2),x + round(w/2) + round(target_width/2)
...
#                     roi = img[my1:my2,0:target_width]
#                 #roi2 = img[y:y+h,x:x+w]
#                 diff = (mx1 - old_x1)/step
#                 print(f"Intial: {old_x1}, Final: {mx1}, diff: {diff}")
#                 for i in range(1,step):
#                     fra = n - step + i
#                     y1,y2,x1,x2 = 0,0+target_height ,old_x1 + round(i*diff),old_x1 + round(target_width)+round(i*diff)
#                     print(f"x1: {old_x1 + round(i*diff)}, x2: {old_x1 + round(target_width)+round(i*diff)} ")
#                     if x1>0:
#                         troi = images[i-1][y1:y2,x1:x2]
#                     else:
#                         troi = images[i-1][y1:y2,0:target_width]
...
#                 # t2 = mx1#+ round(target_width/2)               #final frame
#                 # t1 = old_mx[0] + round(old_mx[2]/2) #initial frame 
#                 # diff = (t2-t1)/step
#                 # print(f'\tInitial x: {t1},Final x: {t2}, adjust per frame: {diff}')
#                 # if n!=1:
...
def analyze_transcript(transcript, retries=100):
    try:
        ...
        return response.choices[0]['message']
         ...
         if retries > 0:
            ...
            return analyze_transcript(transcript, retries=retries-1)
         else:
            raise Exception("OpenAI API Error") from error
from skimage.metrics import structural_similarity
import numpy as np
def checksim(first,second):
    first_gray = cv2.cvtColor(first, cv2.COLOR_BGR2GRAY)
...
# Convert audio from the first video to MP3
def aud(path,sub_path):
    video = VideoFileClip(path)
    video.audio.write_audiofile("Audio.mp3")

    # Load video and audio clips
    video_clip = VideoFileClip(sub_path)
    ...
    final_clip.write_videofile(sub_path, codec="libx264", audio_codec="aac")

def addaud(video_path, music_path):

    # Define the output video path
    output_path = f"{video_path[:-4]}_final.mp4"
    ...
    min_duration = min(video_clip.duration, music_clip.duration)

    # Trim video and audio to the minimum duration
    video_clip = video_clip.subclip(0, min_duration)
    music_clip = music_clip.subclip(0, min_duration)

    # Set the audio of the video clip to the trimmed music
    video_clip = video_clip.set_audio(music_clip)

    # Write the final video with the added music
...
def main():
    video_id='LCoAXH55sNw'
    url = 'https://www.youtube.com/watch?v='+video_id  # Replace with your video's URL
    filename = 'input_video.mp4'
    download_video(url,filename)
    print('downloaded')
    transcript = get_transcript(video_id)
    print('T:Don: ',transcript,len(transcript))
    #interesting_segment = []
    #for a in transcript:
    interesting_segment = analyze_transcript(transcript)
        #print(a,interest)
        #interesting_segment.append(interest)
    print(interesting_segment,len(interesting_segment))
    ...
    for i in range(0, 3):  # Replace 3 with the actual number of segments
        ...
        reeler3(input_file,output_file)
        audio_file = convert_video_to_audio_ffmpeg(input_file, output_ext="mp3")
        addaud(output_file, audio_file)
        #aud(input_file,output_file)
        #print(faces)
